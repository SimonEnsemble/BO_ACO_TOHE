...
We now derive formulas for how the two objectives, the expected team reward $\mathbb{E}[R(\mathcal{P})]$ and the expected number of robots that survive the mission $\mathbb{E}[S(\mathcal{P})]$, depend on the robot-team trail plan $\mathcal{P}$. 
These formulas follow from the directed graph $G$, edge survival probability map $\omega$, and node reward functions $\{r_v : v \in \mathcal{V}\}$.
