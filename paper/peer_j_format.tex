...
\begin{abstract}
Consider a team of mobile [aerial, ground, or aquatic] robots for orienteering in a hazardous environment.
...
These hazards may emanate from e.g.\ rough terrain or seas, strong winds, obstacles, or adversaries capable of attacking or capturing robots.
Now, each node of the graph offers a reward to the team if visited by a robot.
This reward represents the utility gained by the team when a robot e.g. delivers a good, takes an image, or actuates some process to/of/at that node. \\

We wish to obtain the Pareto-optimal set of robot-team trail plans that maximize two [conflicting] team objectives: the expected (i) team reward and (ii) number of robots that survive the mission. Presented with the Pareto-front, a human decision-maker can then inspect the reward-survival tradeoffs then make an informed selection of a Pareto-optimal robot-team trail plan that balances, according to his or her values, reward and robot survival.  \\

To search for the Pareto-optimal set of robot team trail plans, we implement 
(1) bi-objective ant colony optimization, guided by both pheromone and heuristics specific to the problem, and (2) bi-objective simulated annealing. \\

As case studies, we solve and analyze three problem instances: a synthetic one on a two-community graph; an information-gathering mission in an art museum; and an artifact destruction mission at a nuclear power plant.
We find that ant colony optimization consistently outperforms or performs indistinguishably from simulated annealing. Ablating the pheromone trails and heuristics indicates that both are important for guiding ants towards Pareto-optimal robot-team trail plans.
By inspecting a sample of Pareto-optimal robot-team trail plans, we find robots take the safest trails to visit the nodes assigned to them and visit higher-reward nodes earlier in their trail; multiple robots may plan to redundantly visit the same subgraphs to endow the team reward with robustness against robot failures; and we must balance the reward from visiting locations against survival of robots. 
...
Further, we quantify the contribution of (a) the greedy heuristics and (b) the pheromone to the search efficiency of ACO through two ablation studies. 
