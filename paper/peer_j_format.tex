...
(2) real-world graphs modeling (2a) the San Diego art museum and (2b) the [unfinished] Satsop Nuclear Power Plant in Washington.
...
The reward and survival pheromone maps are shared by the ant colony as a form of collective memory of arcs that led to Pareto-optimal trails with high expected reward and survival. Thereby, the ants in the colony collaborate in finding Pareto-optimal robot-team trail plans.
In the trail-construction rule in eqn.~\ref{eq:prob_x_y} and the pheromone-laying rule in eqn.~\ref{eq:tau_update}, each ant uses the same pair of pheromone maps to construct the trail for each robot, too. (Still, when an ant is sequentially allocating trails to the robots, it is aware of the trails $\rho_1, ..., \rho_{k-1}$ of the previous robots when constructing the trail for robot $k$ through the heuristic $\eta_R$ in eqn.~\ref{eq:eta_R}.) This contrasts with an approach of using a distinct, independent pair of pheromone maps for each robot on the team, which requires more memory and conflicts with the inherent permutation-invariance in the homogenous team of robots.
