...
Path-planning algorithms for mobile robots in [static or dynamic] environments \cite{lavalle2006planning,liu2023path,ugwoke2025simulation} are vital for good task performance, efficiency, safety, and autonomy.
For example, a classic path-planning problem involves \cite{lavalle2006planning} finding the shortest collision-free path from a source location to a destination in a static, known environment modeled as a grid, where obstacles render some grid cells inaccessible.
...
These methods trade off computational cost, memory usage, parallelizability, solution optimality, versatility, and scalability \cite{ugwoke2025simulation}.
...
Compared to a single robot, a team can increase spatial coverage and task performance, decrease the time to achieve the objectives, and make achievement of the objectives robust to the failure of robots \cite{schranz2020swarm,brambilla2013swarm}.
For example, in the (NP-hard) team orienteering \cite{golden1987orienteering} problem (TOP) \cite{chao1996team,gunawan2016orienteering,vansteenwegen2011orienteering}, a team of robots are mobile in an environment modeled as a graph, and each node gives a reward to the team if visited by a robot.
...
Solution approaches to the TOP include exact (e.g., a branch-and-price scheme \cite{boussier2007exact}), heuristic \cite{chao1996team}, and meta-heuristic (e.g., ant colony optimization \cite{ke2008ants}) algorithms \cite{vansteenwegen2011orienteering}. 
...
(3) finally, solving and analyzing several examples of BOTOHE problem instances for insights.
...
Each node of the graph offers a reward to the team if a robot visits it.
...
(2) for the reward objective, the robots a) daringly visit dangerous subgraphs to attempt collection of the rewards offered by nodes therein, b) visit the lower-risk and higher-reward subgraphs earlier in the mission, and c) build node-visit redundancy into their trail plans (i.e., multiple robots visit the same node) to make the team-reward robust to the loss of robots during the mission; and
...
To handle the conflict between the reward and survival objectives in the BOTOHE, we search for the Pareto-optimal set \cite{pardalos2017non,branke2008multiobjective} of robot-team trail plans. By definition, a Pareto-optimal trail plan cannot be altered to give a higher expected reward without lowering the expected number of robots that survive---and vice versa. 
...
To search for the Pareto-optimal set of robot-team trail plans in the BOTOHE, we specify and implement a bi-objective ant colony optimization (ACO) algorithm \cite{iredi2001bi}, guided by heuristics.
We adopt the ACO meta-heuristic \cite{dorigo2006ant,bonabeau1999swarm,blum2005ant} because it 
(i) excels at searching for [near-]optimal trail sets on graphs by balancing exploration and exploitation through (a) stochastic decision-making and (b) swarm intelligence where simulated ants collectively learn from experience via laying pheromone trails then follow pheromone trails to leverage that experience;
(ii) allows incorporation of BOTHE-specific heuristics to guide the search and accelerate convergence;
(iii) scales more effectively to larger problems compared to integer program solvers;
(iiii) is embarssingly parallelizable, as each ant can be allocated to an independent thread;
(iv) is straightforward to implement and computationally efficient;
(v) can be extended to handle dynamic or online BOTHE problem variants.

For illustration, we solve and analyze a series of three BOTOHE problem instances for an e.g.\ information-gathering mission on:
(1) a random block graph with three communities;
(2) a real graph modeling (2a) the San Diego art museum and (2b) the Satsop Nuclear Power Plant.
We benchmark bi-objective ACO against random search; quantify the contribution of the greedy heuristics and pheromone to the search efficiency of ACO through two ablation studies; and compare performance of ACO when granted a single pheromone trail (for each objective), shared among robots, versus granting each robot its own pheromone trail.
