...
While an ant travels from a food source back to the nest, it deposits pheromone on the ground. A relatively volatile chemical substance \cite{david2009trail}, pheromone is an olfactory cue for other ants \cite{knaden2016sensory} and thus a means of \emph{indirect} communication between ants. 
...
Since the amount of pheromone an ant deposits on the way back from the nest is modulated by the quality of the food source, ants may also select the highest-quality food sources among multiple options \cite{beckers1993modulation}.
...
To a degree, this stochasticity in the process of ants choosing a trail is beneficial for continual exploration to find new food sources and even shorter paths, 
exploiting multiple food sources in parallel, 
and 
endowing the colony with plasticity in a dynamic environment \cite{deneubourg1983probabilistic,shiraishi2019diverse,deneubourg1986random,dussutour2009noise,edelstein1995trail}.
...
In summary, pheromone trails of an ant colony abstractly constitute a collective memory for the colony, a local guide for ants to high-quality food sources and short paths to them, and a medium through which ants indirectly communicate \cite{jackson2006communication}. 
The positive feedback, negative feedback, and randomness in the behavior of ants---decentralized agents---laying and following pheromone trails enables self-organization of the colony \cite{bonabeau1997self}, where ``complex collective behaviors may emerge from interactions among individuals that exhibit simple behaviors''.
...
A $\lambda$ closer to zero (one) implies the ant prioritizes maximizing the expected reward $\mathbb{E}[R]$ (expected robot survivals $\mathbb{E}[S]$). This heterogeneity in the colony provides some division of labor: different ants tend to focus on finding trail plans in different regions of the Pareto front. 
...
The value $\tau_R(v, v^\prime)$ ($\tau_S(v, v^\prime)$) scores the promise of arc $(v, v^\prime)$, learned from all past experience of the ant colony, for belonging to robot trail plans that maximize the expected reward $\mathbb{E}[R]$ (number of robots that survive $\mathbb{E}[S]$).
...
I.e., the ant has selected the first $i$ arc-hops for robot $k$, giving the \emph{partial} trail $\tilde{\rho_k}=(v_b, \rho_k[1], ..., \rho_k[i]=v)$.
The probability of next hopping to node $v^\prime$ across arc $(v, v^\prime)$ not yet traversed in the partial trail is:
 \begin{equation}
	\pi(v^\prime \mid \rho_1, ..., \rho_{k-1}, \tilde{\rho_k}) \propto 
	 ...
	 \label{eq:prob_x_y}
\end{equation}
The partial trail is more likely to be extended with $\rho_k[i+1]:=v^\prime$ if arc $(v, v^\prime)$ has more pheromone $\tau_{R, S}(v, v^\prime)$ and/or greedy appeal $\eta_{R, S}(v, v^\prime)$---with more or less emphasis on the reward or survivability pheromone and greedy appeal depending on the ant's $\lambda$. 
...
\subparagraph{Heuristics.} The maps $\eta_S: \mathcal{E} \rightarrow \mathbb{R}_+$ and $\eta_R:  \mathcal{E} \rightarrow \mathbb{R}_+$ serve to bias and guide ants' trail-building towards good solutions to accelerate the convergence of the ACO algorithm. Both heuristics are greedy and myopic.
...
Both (i) the subset of ants that found the Pareto-optimal set of robot-team trails $\{\mathcal{P}_1^*, ..., \mathcal{P}_P^*\}$ over that iteration and (ii) the elitist ant who maintains the global-Pareto-optimal set of robot-team trails $\{\mathcal{P}_{P+1}^*, ..., \mathcal{P}_{P+P^\prime}^*\}$ over all iterations, deposit pheromone on the arcs belonging to these trails. The amount of pheromone deposited on arc $(v, v^\prime)$ is:
\begin{equation}
