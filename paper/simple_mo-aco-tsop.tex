...
The node reward map $r: \mathcal{V} \rightarrow \mathbb{R}$ specifies the reward offered to the team by each node. 
...
Note, 
(1) even if a robot is destroyed after leaving node $v$, it still counts as a successful visit to node $v$ to extract its reward;
(2) multiple visits to a node do not provide further marginal reward; and
...
The BOTOHE formulation follows the TSOP \cite{jorgensen2018team} with four modifications: we (i) omit the constraints that each robot survives above a threshold probability\footnote{Ie., we accept if one [unmanned] robot bears much more risk of destruction than another during the mission.}, (ii) allow robots to follow trails instead of paths, allowing a robot to visit a given node more than once and to orienteer in a directed graph containing nodes with an in- and out-degree of one\footnote{E.g. paths prohibit a robot from visiting multiple nodes in a star graph.}, (iii) aim to maximize two objectives instead of one, and (iv) do not assume the team reward viewed as a function over the set of visited nodes is monotone because we allow for negative rewards.
...
\begin{multline}
	\pi[S=s] = \sum_{\substack{\mathcal{S} \subseteq \{1, ..., K\}  \\ \lvert \mathcal{R} \rvert = s} } \,
...
Now, let $T_v(\mathcal{P}=\{\rho_1, ..., \rho_K\} )$ be the Boolean random variable that is one if one or more robots on the team with trail plans $\mathcal{P}$ visit node $v$ and zero otherwise (i.e., zero robots on the team visit node $v$).
The latter occurs if and only if none of the robots visit node $v$. Hence,
\begin{equation}
...
\end{equation} where the reward from node $v$ is collected if and only if node $v$ is visited by one or more robots (i.e. iff $T_v(\mathcal{P})=1$).
