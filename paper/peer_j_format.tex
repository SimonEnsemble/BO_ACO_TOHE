...
Extending to multiple robots, each robot may have different start and end locations, and robot-robot collisions must be avoided as well.
Multi-robot path planning \cite{wagner2011m} deals with a much larger configuration space.

For many applications, we wish for a team of multiple mobile robots to coordinate their routes in a large-scale environment to cooperatively achieve a shared, team-level objective \cite{parker1995design,parker2007distributed}.
Compared to a single robot, a robot team can increase spatial coverage, enhance the performance on the high-level task at hand, decrease the time to achieve the objectives, and make achievement of the objectives robust to the failure of some robots \cite{schranz2020swarm,brambilla2013swarm}.
Multi-robot, coordinative routing, framed as a combinatorial optimization problem, involves high-level, strategic decision-making where fine-grained details like obstacles in the environment are abstracted away.
% Multi-robot, coordinative routing problems are framed as combinatorial optimization problems.
For example, in the team orienteering \cite{golden1987orienteering} problem (TOP) \cite{chao1996team,gunawan2016orienteering,vansteenwegen2011orienteering}, a team of robots are mobile in a large-scale environment modeled as a graph, and each node gives a reward to the team if visited by a robot.
...
(i) anticipates failures and makes risk-aware route plans (e.g., redundantly assigning multiple robots to visit a location) that endow the team with \emph{robustness}---the ability to withstand failures with minimal concession of the objective,
and/or
...
A few route-planning problems for robots orienteering in hazardous environments (abstracted as graphs) have been framed \cite{zhou2021multi}. 
