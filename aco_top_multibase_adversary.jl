...
function _draw_nodes!(
	...
	highlight_node_list::Vector{Int}=Int[],
	mark_bases::Bool=true
)
	ax.xlabel = "[length]"
	ax.ylabel = "[length]"
...
function viz_soln(
	...
	_draw_nodes!(fig, ax, top, mark_bases=false)
	Legend(
		fig[2, 1], line_plots, ["robot $k" for k = 1:top.nb_robots], 
		orientation=:horizontal, tellwidth=false, tellheight=true,
		labelsize=12
	)
	# draw robots's ENDING positions
	for robot_id = 1:top.nb_robots
		# u -> v = last step.
		x = 0.1 * top.X[:, soln.routes[robot_id][end-1]] + 
			0.9 * top.X[:, soln.routes[robot_id][end]]
		scatter!([x[1]], [x[2]], markersize=25, marker='✈')
	end
...
function viz_trajectory(aco_res::ACOResult, baseline_fitness::Float64; ylimits=(0, 1))
	...
	Label(fig[1, 1], 
		"global best fitness=$(round(aco_res.global_best_fitness, digits=3))", 
		tellwidth=false, tellheight=false, halign=0.9, valign=0.1,
		font=firasans("light"), fontsize=12
	)
	fig
end
