...
In metallurgy and materials science, annealing is the process of heating a crystalline material to a high temperature, then cooling it to improve its crystallinity (i.e., reduce defects) and, thereby, some property (e.g., ductility).

Now, from a statistical mechanics \cite{chandler1987introduction} perspective, a material is an atomistic system characterized by an energy landscape. This energy landscape assigns an energy to each micro-state of the system. A micro-state constitutes the configuration (i.e., positions) of the atoms in the system as well as their momenta. The energy landscape of realistic atomistic systems is high-dimensional and multi-modal.

At higher temperatures, atomistic systems more broadly explore the energy landscape. Thanks to fluctuations, the system can climb energy barriers and visit many different basins hosting different local minima.
Higher-energy micro-states are commonly visited, and the system is more disordered.
On the other hand, at lower temperatures, the system typically becomes trapped in a basin corresponding with some local minimum and tends to exhibit more order.
...
As the system is cooled, the system settles into some basin of the energy landscape hosting a lower-energy local minimum corresponding with more order (i.e., fewer defects).
  
Simulated annealing (SA) \cite{kirkpatrick1983optimization,simon2013evolutionary,zomaya2010simulated}  makes an analogy between the energy landscape of an atomistic system and an objective function to stochastically search for a global-optimal solution to a multi-modal optimization problem.
  SA borrows from Metropolis Markov chain Monte Carlo simulation of an atomistic system under the canonical statistical mechanical ensemble \cite{frenkel2023understanding} while decreasing the temperature as the search proceeds to mimic cooling. 
  When the temperature is high at the beginning of the search, SA is highly exploratory and entertains a wide range of diverse solutions.
  As the temperature decreases, SA settles into a basin of the objective function---hopefully, a basin containing a good extremum of the objective function. At lower temperatures, SA is highly exploitative, in that it explores solutions only in the vicinity of the current-best solution.
  
  To apply an SA algorithm for a given optimization problem, our tasks are to devise \cite{simon2013evolutionary}: 
  (1) a method to randomly sample a solution that neighbors the current solution e.g.\ by randomly perturbing the current solution; and 
  (2) the cooling rate, i.e.\ the schedule by which temperature decreases over iterations.
    
\paragraph{Bi-objective SA of the TOHE problem.}
...
\end{equation} with $w_r \in [0, 1]$ the dynamic weight on the reward objective. 
We choose the notation $E(\mathcal{P})$ to make the analogy with the energy of an atomistic system; here, the robot-team trail plan $\mathcal{P}$ is analogous to the configuration of the atomistic system.

SA to minimize $E(\mathcal{P})$ for a fixed $w_r$ evolves a single robot-team trail plan over many iterations, beginning with an initial robot-team trail plan $\mathcal{P}_0$.
At each iteration, we 
   (1) randomly perturb the current solution $\mathcal{P}$ to obtain a new, neighboring solution $\mathcal{P}^\prime$ then 
   (2) decide to accept or reject this perturbation based on the energy change $\Delta E:= E(\mathcal{P^\prime})- E(\mathcal{P})$ and temperature $T$ then   
   (3) reduce the temperature to decrease exploration. 
Additionally, we track the minimum-energy team trail plan $\mathcal{P}^*$ observed so far. At the beginning of the iteration, with probability $p_{\rm reset}=0.01$, we replace the current solution $\mathcal{P}$ with $\mathcal{P}^*$. 
...
We sample a neighboring robot-team trail plan $\mathcal{P}^\prime$ by randomly perturbing each robot's trail plan in $\mathcal{P}$, through random selection of: (i) insert a random new node into the trail, (ii) delete a random node in the trail, (iii) swap two random nodes in the trail, (iv) substitute a random node in the trail with another random node; (v) delete a random segment of the trail, or (vi) reverse the trail. Unlike working with paths in a complete graph, some coding effort was required to make perturbations that give a feasible trail on the graph. 
...
\vspace{-\baselineskip}
\subparagraph{Incrementing the weight $w_r$ in the scalarized objective function and resetting the temperature.}
To search for the Pareto-optimal set of robot-team trail plans with a single-objective SA algorithm, we periodically and concurrently 
(1) incrementally increase the weight $w_r$ on the expected reward in the scalarized objective function in eqn.~\ref{eq:energy};
(2) reset the temperature to the initial [high] temperature $T_0$ to increase exploration, now that the objectives are weighted differently; and
(3) replace the current robot-team trail plan with the lowest-energy (according to the previous $w_r$) one observed over the course of the recent SA episode.
With a set budget of $i^2$ total iterations, we run through a sequence of $i$ weights $w_r$ equally-spaced on $[0, 1]$ to scan the Pareto-front. For each $w_r$, run a single-objective SA for $i$ iterations. 
We begin with $w_r=0$ and an initial trail plan $\mathcal{P}_0$ where all robots stay at the base node for maximal survival.
The approximate Pareto-optimal set obtained by our bi-objective SA approach is the Pareto-optimal subset of the $i$ optimal robot-team trail plans found from each episode of SA of a scalarized objective function.
Note, gradually incrementing $w_r$ and starting each single-objective SA episode with the robot-team trail plan from the previous episode for a warm start allows us to initialize with a quite low temperature $T_0$; after all, not as much exploration should be needed, compared to restarting with a random robot-team trail plan for each scalarized objective function. 

\vspace{-\baselineskip}
\subparagraph{Comparing bi-objective ACO and SA.}
