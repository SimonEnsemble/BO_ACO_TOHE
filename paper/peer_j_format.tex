...
\item ACO with heuristics ablated, where $\eta_{R, S}(v, v^\prime):=1$ for all arcs $(v, v^\prime)\in\mathcal{E}$, to quantify the contribution of the heuristics for guiding the search;
\item ACO with pheromone ablated, where statically $\tau_{R, S}=1$ for all arcs, to quantify the contribution of the pheromone for guiding the search;
\item a [naive] random search where $\eta_{R, S}=\tau_{R,S}:=1$ ;
...
We set the pheromone evaporation rate at $\rho=0.1$. To handle different numbers of robots and scales of reward for different problems, we initialize the pheromone maps with, for each arc $(v, v^\prime)$, $\tau_s(v, v^\prime) := K$ and $\tau_r(v, v^\prime) := \sum_{v \in \mathcal{V}} r(v)$.

\paragraph{Simulated annealing baseline}
We implemented a bi-objective simulated annealing (SA) algorithm \cite{kirkpatrick1983optimization,simon2013evolutionary} for a competitive baseline against which to benchmark ACO.
...
From a statistical thermodynamic perspective:
  at high temperatures, the system is highly disordered as it explores high-energy micro-states; as the system cools, it settles into a basin of low-energy micro-states that exhibit more order.
  The fluctuations at higher temperatures are important to overcome energy barriers; hopefully, the system hops into the lowest-energy basins of the micro-state space corresponding to highly crystalline materials as the system cools.
  
Taking inspiration from annealing of crystalline materials, simulated annealing (SA) makes an analogy between the energy of an atomistic system and an objective function to search for global-optimal solutions in a combinatorial optimization problem.
  SA directly mimics a Metropolis Markov chain Monte Carlo simulation of the canonical statistical thermodynamic ensemble governing the micro-states of a crystalline material at fixed temperature, volume, and particle number while decreasing the temperature as the search proceeds.
  As opposed to ACO, SA maintains a single robot team-trail plan, and iteratively evolves it. The team-trail plan is analogous to the micro-state of a simulated crystalline system. For each step of the SA, instead of proposing to perturb the position(s) of atom(s) in a simulated physical system, we propose to perturb a robot-team trail plan by, for each robot: (i) inserting a new node into the trail, (ii) deleting a node in the trail, (iii) swapping two nodes in the trail, (iv) substituting a node in the trail with another node; (v) deleting a segment of the trail, and (vi) reversing the trail. Then, after generating this feasible ``neighboring'' (i.e., slightly perturbed) trail-plan at random, we compute its expected reward and robot survival.
...
We obtained the two-community graph via a sample from a stochastic block model (community \#1: 8 nodes, community \#2: 12 nodes; probability of community \#1-\#1, \#2-\#2, and \#1-\#2 joins 0.3, 0.4, 0.025, respectively).
At the base node, $K=2$ robots begin.
We assigned rewards to community \#1 and \#2 by sampling from two different Gaussian distributions ($\mu_1=5$, $\sigma_1=2$; $\mu_2=1$, $\sigma_2=1/2$, both truncated below zero).
For arc survival probabilities, we sampled from the same Gaussian ($\mu=0.9$, $\sigma=0.3$, truncated below zero and above one).

\paragraph{Search progress.}
Fig.~\ref{fig:two_community}B shows the search progress of ACO in terms of the normalized area indicator over iterations. After 10\,000 iterations, the area indicator for ACO plateaus. By comparison, ACO without the heuristics and ACO without pheromone trails perform quite poorly, but better than random search. 
More, ACO outperforms the competitive SA baseline.
Reflecting the competitiveness of SA, at 1\,000 and 10\,000 iterations, the SA baseline outperforms ACO without the heuristic and without the pheromone.  
...
Finally, Fig.~\ref{fig:two_community}D shows the learned pheromone trails that guided the search in a run of ACO after 10\,000 iterations.
