...
Choosing a path-planning algorithm usually trades off solution quality, computational cost, memory usage, parallelizability, versatility, and scalability \cite{ugwoke2025simulation}.
In multi-robot path planning \cite{wagner2011m}, each robot has unique start and end locations; robot-robot collisions must be avoided as well; and the configuration space is much larger.
...
Compared to a single robot, a robot team can increase spatial coverage, enhance performance on the high-level task at hand, achieve the objectives more quickly, and make achievement of the objectives robust to failure of some robots \cite{schranz2020swarm,brambilla2013swarm}.
Multi-robot, coordinative routing involves high-level, strategic decision-making, where fine-grained details like obstacles in the environment and precise spatial paths are abstracted away.
...
A prominent example of a multi-robot, coordinative routing problem is the team orienteering \cite{golden1987orienteering} problem (TOP) \cite{chao1996team,gunawan2016orienteering,vansteenwegen2011orienteering}.
...
(i) anticipates failures and makes risk-aware route plans that endow the team with \emph{robustness}---the ability to withstand failures with minimal concession of the objective,
and/or
(ii) changes their route plans during the mission, adapting in response to realized robot failures, to recoup the otherwise anticipated loss in the objective. 
For example, a risk-aware route plan may redundantly assign multiple robots to visit a given location to carry out a task that requires only one robot.
...
The offline version of RMOP constitutes a two-stage, sequential game with perfect information: (1) the robot team chooses a set of paths then (2) the adversary, knowing these paths, chooses the subset of robots to attack and destroy. 
...
Other work involving robot route-planning in hazardous environments includes 
...
Though not a robot routing problem, we must mention the time-bomb knapsack problem \cite{monaci2022exact}: maximize the expected profit of items placed in a knapsack under a capacity when each item has a probability of exploding and causing the profit to be lost.
...
As opposed to aggregating the two objectives into a single, scalar objective, a Pareto front (i) reveals the tradeoffs between expected rewards and robot survivals and (ii) allows us to delay in placing quantitative preferences on the two objectives.
Later, a human decision-maker can examine the tradeoffs by visualizing the Pareto front then make an informed selection of a Pareto-optimal robot-team trail plan that suites their preferences/values.
...
In ACO, a simulated colony of decentralized ants construct trails stochastically over iterations while 
...
(a) exploitation of the colony's memory of good trails via biasing towards arcs with high pheromone and 
...
ACO handles our bi-objective problem by assigning ants to specialize to different regions of the Pareto front and maintaining two species of pheromone---one for each objective \cite{iredi2001bi}.
