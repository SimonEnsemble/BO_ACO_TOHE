...
The BOTOHE formulation follows the TSOP \cite{jorgensen2018team} with four modifications: we 
...
\section{Results}
We now implement and benchmark bi-objective ant colony optimization to search for Pareto-optimal robot-team trail plans for an instance of a team orienteering in a hazardous environment. We visualize some of the solutions to gain intuition. 

\paragraph{Problem setup.}
\subparagraph{The information-gathering mission in an art museum.} 
...
Generally, the command center wishes (i) to collect images of the art in each gallery and (ii) for the [expensive] robots to return from the mission. 
...
The museum is partitioned into a collection of 23 distinct art galleries (rooms). The galleries are connected to each other via doorways or hallways in a rather rich pattern.
...
We construct a set of arcs $\mathcal{E}$ according to the existence of direct doorways between the rooms. For each pair of rooms connected by a doorway, we allocate two arcs in each direction. 

\subparagraph{The arc-traversal hazards.}
Suppose, owing to 
(i) adversaries in the museum that seek to prevent the robots from imaging the art and 
(ii) obstacles in the museum that the robots could crash into,
each arc-traversal by a robot incurs a risk of robot destruction and capture by the adversary. 
We construct the arc-survival probability map $\omega$ giving survival probabilities for the arc(s)
...
Each node represented a gallery gives a reward to the robot team if visited by one or more robots. Here, we assume upon visiting a node, the robot images the art in the gallery and transmits the image back to the command center---giving utility to the command center represented by the value of the node-rewards. We construct the node reward map $r$ giving a reward of
...
\begin{figure}[h!]
    ...
    (b) The pheromone trails and distribution of the amount of pheromone on the arcs at the end of the ACO. 
    (c) The [approximate] Pareto-front of robot-team team trail plans with a few select plans highlighted.
    }
\end{figure}

\paragraph{ACO hyperparameters.}
We use a colony of $N_{ants}=100$ ants and an evaporation rate of $\rho=0.05$. We run ACO for 10\,000 iterations.
