...
(1) for the survival objective, risk-averse robots (a) avoid visiting dangerous subgraphs despite the rewards offered and (b) take the safest closed trails to visit the nodes assigned to them;
(2) for the reward objective, the daring robots (a) visit dangerous subgraphs to attempt collection of the rewards offered, (b) visit the lower-risk and higher-reward subgraphs earlier, and (c) build node-visit redundancy into their trail plans to make the team-reward robust to the loss of robots during the mission; and
...
The arc survival probability map $\omega: \mathcal{E} \rightarrow (0, 1]$ is known, static over the course of the mission, and not necessarily symmetric (owing to eg., i) [directional] air or water currents or bright sunlight or ii) an adversary with limited range nearer one node than another).
...
(2) multiple visits to a node by the same or distinct robot(s) do not provide marginal reward over a single visit. % I.e.\ the reward from a node is irrevocably collected by the team [only] by the first robot to visit it.
%; and
...
(1) the expected team \underline{r}eward, $\mathbb{E}[R]$, and (2) the expected number of robots that \underline{s}urvive the mission, $\mathbb{E}[S]$. (Both $R$ and $S$ (i) are random variables owing to the stochasticity of robot survival while trail-following and (ii) depend on the robot-team trail plan $\mathcal{P}$ owing to different rewards among nodes and dangers among arcs.) (A robot \emph{survives} the mission if it visits all nodes in its planned trail and returns to the base node [without getting destroyed].)
...
The Pareto-optimal set of team trail plans has value for presenting a \emph{portfolio} of team trail plans to a human decision-maker, who ultimately invokes their values---i.e., makes a team-reward vs.\  robot-survival tradeoff---by selecting a good team trail plan from the Pareto set.
...
\end{align}
That is, a Pareto-dominant plan $\mathcal{P}^\prime$ is not worse than plan $\mathcal{P}^*$ for reward nor for survivability, but is better for one of them.
...
and
(iii) have two objectives instead of one and seek the Pareto-optimal set.
...
(i) pheromone evaporates over time \cite{jackson2006communication,david2009trail,van2011temperature} so the ant colony can ``forget'' trails to exhausted food sources
and 
(ii) ants deposit less pheromone on trails (i) with already high pheromone concentrations \cite{czaczkes2013ant} or (ii) leading to food sources already occupied by their nestmates \cite{wendt2020negative}.
...
Some species of ants can deposit multiple species of pheromone (from different glands) with eg. different longevities \cite{czaczkes2015trail}, allowing for more complex indirect communication \cite{jackson2006communication} eg. a ``no entry'' signal \cite{robinson2005no}. 
...
Owing to positive feedback, negative feedback, and randomness in [decentralized] ants laying and following pheromone trails, ``complex collective behaviors may emerge from interactions among individuals that exhibit simple behaviors''  \cite{bonabeau1997self}. \cite{bonabeau1999swarm,goss1989self,jackson2006communication,edelstein1995trail,watmough1995modelling}
...
In bi-objective ACO, we simulate a heterogenous colony of artificial ants laying and stochastically-following trails of two different species of pheromone (one for each objective) on the graph $G$, over many iterations. 
At each iteration, each ant stochastically constructs a robot-team trail plan $\{\rho_1, ..., \rho_K\}$ robot-by-robot, arc-by-arc, biased by (i) the amounts of two species of pheromone on the arcs encoding the colony's past experiences and (ii) two heuristics that score the greedy, \emph{a priori} appeal of each arc for each objective. 
Each ant in the colony specializes to a different region of the Pareto-front.
Then, ants that found the Pareto-optimal team trail plans over that iteration deposit pheromone of each species on these arcs, proportionally to the value of the associated objective.
...
\subsection{The heterogenous artificial colony of ants}
We construct a heterogenous colony of $N_{\text{ants}}$ artificial ants.
Ant $i\in\{1, ..., N_{\text{ants}}\}$ in the colony is assigned a parameter $\lambda_i := (i-1) / (N_{\text{ants}}-1)$ dictating its balance of the two objectives when searching for Pareto-optimal team trail plans.
...
As division of labor, different ants seek trail plans belonging to different regions of the Pareto front. 
Additionally, an elitist ant maintains the global-Pareto-optimal set of team trail plans.
...
(i) amounts of two distinct species of artificial pheromone, $\tau_R(v, v^\prime)$ and $\tau_S(v, v^\prime)$, and 
(ii) heuristic scores $\eta_R(v, v^\prime)$ and $\eta_S(v, v^\prime)$.
Both $\tau_{R,S}(v, v^\prime)$ and $\eta_{R,S}(v, v^\prime)$ score the promise of arc $(v, v^\prime)$ for belonging to Pareto-optimal team trail plans that maximize $\mathbb{E}[R]$ and $\mathbb{E}[S]$, respectively, and guide ants' construction of robot team trail plans.
The pheromone reflects learned past experience of the ant colony---a memory of the propensity the arcs to belong to trails contained in Pareto-optimal team trail plans and the objective values associated with those plans---and is dynamic over iterations due to deposition by the ants and evaporation. 
...
% Together, the pheromone maps $\tau_{R,S}$ represent a [fading] memory of the artificial ant colony about the propensity of each arc to belong to Pareto-optimal robot-team trail plan and, vaguely, where along the Pareto-front the plan might lie.
% vaguely, through laying and following \emph{two} species of pheromone, .

\subsection{Constructing robot-team trail plans}
Each iteration, every ant in the colony stochastically constructs a team trail plan $\mathcal{P}=\{\rho_1, ..., \rho_K\}$ by sequentially allocating trails to the robots and [conceptually] following the closed trail the ant designs for the robot.
...
Robot trail construction is achieved by iteratively applying a stochastic, partial trail extension rule. Suppose an ant with objective-balancing parameter $\lambda$ is constructing the closed trail for robot $k$, $\rho_k$, and currently resides at node $v=\rho_k[i]$.
...
Through this process, the ant colony generates a set of robot-team trail plans, then we compute the associated objectives via eqns.~\ref{eq:formula_obj2} and \ref{eq:formula_obj1}, giving $\{ (\mathcal{P}_i, \mathbb{E}[R(\mathcal{P}_i)], \mathbb{E}[S(\mathcal{P}_i)])\}_{i=1}^{N_{\text{ants}}}$.
...
\end{equation}
with $\mathcal{I}$ the indicator function---myopic because it does not account for rewards robot $k$ could collect further along the trail. 
...
Deposition, accomplished by the second term in eqn.~\ref{eq:tau_update}, constitutes indirect communication to ants in future iterations about which arcs tend to belong to Pareto-optimal team trail plans.
First, the ants \emph{collaborate} by 
(i) comparing the objectives achieved by the team trail plans they constructed \emph{this} iteration to obtain an \emph{iteration}-Pareto-optimal set of plans and 
(ii) sharing the team trail plans they constructed this iteration, and the associated objectives, to the elitist ant who then updates the \emph{global}-Pareto-optimal set.
The iteration- and global-Pareto-optimal team trail plans are amalgamated into a multiset $\{(\mathcal{P}_i^*, \mathbb{E}[R(\mathcal{P}_i^*)], \mathbb{E}[R(\mathcal{P}_i^*)])\}_{i=1}^P$.
Now, each time arc $(v, v^\prime)$ is traversed in a trail in $\mathcal{P}^*_p$, 
an ant deposits pheromone on it in proportion to the objective value associated with $\mathcal{P}^*_p$:
\begin{equation}
	 \Delta \tau_{R,S}(v, v^\prime) :=\sum_{p=1}^{P} \mathbb{E}[R(\mathcal{P}^*_p), S(\mathcal{P}^*_p)] 
...
ACO searches for Pareto-optimal robot-team trail plans guided by (i) the static heuristics $\eta_R$ and $\eta_S$ that greedily score the appeal of traversing a given arc and (ii) the dynamic pheromone maps $\tau_R$ and $\tau_S$ that encapsulate the memory of the ant colony over previous iterations. Here, we quantify how each of these components are contributing to the effectiveness of ACO by ablating each. 
